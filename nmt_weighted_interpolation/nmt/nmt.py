...
Usage:
    nmt.py train --lr_train-src=<file> --hr_train-src=<file> --lr_train-tgt=<file> --hr_train-tgt=<file> --lr_dev-src=<file> --hr_dev-src=<file> --lr_dev-tgt=<file> --hr_dev-tgt=<file> --vocab=<file> [options]
...
Options:
    ...
    --cuda                                  use GPU
    --lr_train-src=<file>                   LR train source
    --hr_train-src=<file>                   HR train source
    --lr_train-tgt=<file>                   LR train source
    --hr_train-tgt=<file>                   HR train source
    --lr_dev-src=<file>                     LR dev source
    --hr_dev-src=<file>                     HR dev source
    --lr_dev-tgt=<file>                     LR dev source
    --hr_dev-tgt=<file>                     HR dev source
    ...
    --low_resource_penalty=<float>          penalty to multiply if batch is a low_resource batch [default: 0.7]
    --valid_on_low_resource=<str>           Should Validation be done on LR data or HR data [default: True]
"""
...
#        print('input type', type(source))
    ...
    def evaluate_ppl(self, dev_data: List[Any], batch_size: int=32):
        ...
        for src_sents, tgt_sents in dev_batch_iter(dev_data, batch_size):
...
def train(args: Dict[str, str]):
    lr_train_data_src = read_corpus(args['--lr_train-src'], source='src')
    lr_train_data_tgt = read_corpus(args['--lr_train-tgt'], source='tgt')

    hr_train_data_src = read_corpus(args['--hr_train-src'], source='src')
    hr_train_data_tgt = read_corpus(args['--hr_train-tgt'], source='tgt')

    lr_dev_data_src = read_corpus(args['--lr_dev-src'], source='src')
    lr_dev_data_tgt = read_corpus(args['--lr_dev-tgt'], source='tgt')

    hr_dev_data_src = read_corpus(args['--hr_dev-src'], source='src')
    hr_dev_data_tgt = read_corpus(args['--hr_dev-tgt'], source='tgt')


    # ---------------------

    lr_train_data = list(zip(lr_train_data_src, lr_train_data_tgt))
    lr_dev_data = list(zip(lr_dev_data_src, lr_dev_data_tgt))

    hr_train_data = list(zip(hr_train_data_src, hr_train_data_tgt))
    hr_dev_data = list(zip(hr_dev_data_src, hr_dev_data_tgt))
    ...
    low_resource_penalty = float(args['--low_resource_penalty'])
    lr = float(args['--lr'])
    ...
    while True:
        ...
        for src_sents, tgt_sents, type_of_batch in batch_iter(lr_train_data, hr_train_data, batch_size=train_batch_size, shuffle=True):
            ...
            if type_of_batch == "lr":
                loss*=low_resource_penalty
            else:
                loss*=(1-low_resource_penalty)
            ...
            if train_iter % valid_niter == 0:
                ...
                dev_data = lr_dev_data if args['--valid_on_low_resource'] else hr_dev_data
