...
def make_model(src_vocab, tgt_vocab, embed_matrix_src, embed_matrix_tgt, emb_size=256, hidden_size=512, num_layers=1, dropout=0.1):
    ...
    model = EncoderDecoder(
        ...
        Generator(hidden_size, tgt_vocab), use_pretrained=True, embed_matrix_src=embed_matrix_src, embed_matrix_tgt=embed_matrix_tgt)
...
def main():
    ...
    model_save_path = os.path.join('experiments/saved_models/', 'model_concat_embed')
...
#    model.load_state_dict(torch.load(model_save_path))    
    ...
    #write_preds(test_data_src, model, vocab, 'mono.txt', 70)
