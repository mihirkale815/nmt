...
def train(model, vocab, train_data, valid_data, test_data, test_data_src, model_save_path, num_epochs=30, lr=0.001, print_every=10):
    ...
    while True:
        ...
        if epoch == num_epochs:
            ...
            write_preds(test_data_src, model, vocab, 'decode_be_concat_embed.txt', 70)
            exit(0)
...
def main():
    ...
    valid_niter = 180
    log_every = 10
    model_save_path = os.path.join('experiments/', 'model_be_concat_embed')
    vocab = pickle.load(open('data/vocab.bin', 'rb'))
    embed_matrix_src = create_embed_matrix('data/wiki.be.vec', vocab.src)
